...
function leopet_meta( $id = '' ){
    
    $value = get_post_meta( get_the_ID(), '_leopet_'.$id, true );
    
    return $value;
}
// Blog Date Permalink
function leopet_blog_date_permalink(){
...
// Blog Excerpt Length
if ( ! function_exists( 'leopet_excerpt_length' ) ) {
	function leopet_excerpt_length( $limit = 30 ) {
...
if ( ! function_exists( 'leopet_posted_comments' ) ) :
    function leopet_posted_comments(){
        
        $comments_num = get_comments_number();
        if( comments_open() ){
            if( $comments_num == 0 ){
                $comments = esc_html__('No Comments','leopet');
            } elseif ( $comments_num > 1 ){
                $comments= $comments_num . esc_html__(' Comments','leopet');
            } else {
                $comments = esc_html__( '1 Comment','leopet' );
            }
            $comments = '<a href="' . esc_url( get_comments_link() ) . '">'. $comments .'</a>';
        } else {
            $comments = esc_html__( 'Comments are closed', 'leopet' );
        }
...
function leopet_iframe_match(){   
    $audio_content = leopet_embedded_media( array('audio', 'iframe') );
    $iframe_match = preg_match("/\iframe\b/i",$audio_content, $match);
    return $iframe_match;
}

//Post embedded media
function leopet_embedded_media( $type = array() ){
...
// WP post link pages
function leopet_link_pages(){
    
    wp_link_pages( array(
    'before'      => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages:', 'leopet' ) . '</span>',
    'after'       => '</div>',
    'link_before' => '<span>',
    'link_after'  => '</span>',
    'pagelink'    => '<span class="screen-reader-text">' . esc_html__( 'Page', 'leopet' ) . ' </span>%',
    'separator'   => '<span class="screen-reader-text">, </span>',
    ) );
}

// Blog social sharing buttons
function leopet_social_sharing_buttons( $ulClass = '', $tagLine = '' ) {
...
if( ! function_exists('leopet_blog_single_post_navigation') ) {
	function leopet_blog_single_post_navigation() {
				...
				<div class="row">
					<div class="col-lg-6 col-md-6 col-12 nav-left flex-row d-flex justify-content-start align-items-center">
						...
						if( get_next_post_link() ){
							...
							if( has_post_thumbnail() ){
								...
								<div class="thumb">
									<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
										<?php echo get_the_post_thumbnail( absint( $nextPost->ID ), 'leopet_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
									</a>
								</div>
								<?php
							} ?>
							...
							<div class="detials">
								<p><?php echo esc_html__( 'Prev Post', 'leopet' ); ?></p>
								<a href="<?php the_permalink(  absint( $nextPost->ID )  ) ?>">
					...
					<div class="col-lg-6 col-md-6 col-12 nav-right flex-row d-flex justify-content-end align-items-center">
						...
						if( get_previous_post_link() ){
							...
							?>
							<div class="detials">
								<p><?php echo esc_html__( 'Next Post', 'leopet' ); ?></p>
							...
							<div class="thumb">
								<a href="<?php the_permalink(  absint( $prevPost->ID )  ) ?>">
									<?php echo get_the_post_thumbnail( absint( $prevPost->ID ), 'leopet_np_thumb', array( 'class' => 'img-fluid' ) ) ?>
								</a>
							</div>
							<?php
						} ?>
...
// Leopet Comment Template Callback
function leopet_comment_callback( $comment, $args, $depth ) {
				...
				<div class="desc">
					...
					<div class="d-flex justify-content-between">
						<div class="d-flex align-items-center">
							<h5 class="comment_author"><?php printf( __( '<span class="comment-author-name">%s</span> ', 'leopet' ), get_comment_author_link() ); ?></h5>
							<p class="date"><?php printf( __('%1$s at %2$s', 'leopet'), get_comment_date(),  get_comment_time() ); ?><?php edit_comment_link( esc_html__( '(Edit)', 'leopet' ), '  ', '' ); ?> </p>
							<?php if ( $comment->comment_approved == '0' ) : ?>
								<em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'leopet' ); ?></em>
								<br>
							<?php endif; ?>
						</div>
...
add_filter('comment_reply_link', 'leopet_replace_reply_link_class');
function leopet_replace_reply_link_class( $class ) {
...
// theme logo
function leopet_theme_logo( $class = '' ) {

    $siteUrl = home_url('/');
    // site logo
		
	$custom_logo_id = get_theme_mod( 'custom_logo' );
	$imageUrl = wp_get_attachment_image_src( $custom_logo_id , 'leopet_site_logo_142x36' );
	
	if( !empty( $imageUrl[0] ) ){
		$siteLogo = '<a class="'.esc_attr( $class ).'" href="'.esc_url( $siteUrl ).'"><img src="'.esc_url( $imageUrl[0] ).'" alt="'.esc_attr( leopet_image_alt( $imageUrl[0] ) ).'"></a>';
	}else{
...
function leopet_pull_right( $id = '', $condation ){
...
function leopet_get_textareahtml_output( $content ) {
...
function leopet_get_slider_shortcode_options( $field ) {
...
// html Style tag
function leopet_inline_bg_img( $bgUrl ){
    $bg = '';
...
function leopet_sidebar_opt(){

    $sidebarOpt = leopet_opt( 'leopet-blog-sidebar-settings' );
    $sidebar = '1';
...
function leopet_blog_pagination(){
	echo '<nav class="blog-pagination justify-content-center d-flex">';
        the_posts_pagination(
            array(
                'mid_size'  => 2,
                'prev_text' => __( '<span class="ti-angle-left"></span>', 'leopet' ),
                'next_text' => __( '<span class="ti-angle-right"></span>', 'leopet' ),
                'screen_reader_text' => ' '
            )
        );
	echo '</nav>';
...
// Themify Icon
function leopet_themify_icon(){
    return[
        'flaticon-layers'     => __('Layer Icon', 'leopet'),
        'flaticon-design'     => __('Design Icon', 'leopet'),
        'flaticon-speaker'    => __('Speaker Icon', 'leopet'),
    ];
}

// CTA section
function leopet_get_cta_area() {
	$cta_section_title      = !empty( leopet_opt( 'leopet_cta_section_title') ) ? leopet_opt( 'leopet_cta_section_title') : '';
	$cta_section_text       = !empty( leopet_opt( 'leopet_cta_section_text') ) ? leopet_opt( 'leopet_cta_section_text') : '';
	$cta_section_btn_label  = !empty( leopet_opt( 'leopet_cta_section_btn_label') ) ? leopet_opt( 'leopet_cta_section_btn_label') : '';
	$cta_section_btn_url    = !empty( leopet_opt( 'leopet_cta_section_btn_url') ) ? leopet_opt( 'leopet_cta_section_btn_url') : '';
	?>
    <!-- cta-area-start -->
...
function leopet_get_contact_details_area() {
	?>
    <!-- location-area-start -->
    <div class="addres-area black-bg section-padding">
        <div class="container">
            <div class="row">
                <?php
				// Contact items
				$contact_items = leopet_opt('leopet_contact_details_section_fields');
                if ( $contact_items == true ) {
                    ...
                    if( is_array( $contact_items ) && count( $contact_items ) > 0 ){
                        foreach ($contact_items as $value) {
                            switch ($counter) {
                                case $counter == 2:
                                    $icon_img = LEOPET_DIR_ICON_IMG_URI . 'clock.png';
                                    break;
                                case $counter == 3:
                                    $icon_img = LEOPET_DIR_ICON_IMG_URI . 'envelope.png';
                                    break;
                                default:
                                    $icon_img = LEOPET_DIR_ICON_IMG_URI . 'globe.png';
                                    break;
                            }
                            $counter++;
