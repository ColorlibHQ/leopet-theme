...
function leopet_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'leopet' ) . '</span></li>';
...
// Blog categories
function leopet_featured_post_cat(){
...
// social media
if ( ! function_exists( 'leopet_social' ) ) {
	function leopet_social( $args = array() ) {
		...
		$url = leopet_opt( 'leopet_social_url' );
...
// header cart count
function leopet_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( LEOPET_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'leopet' ); ?>">
...
function leopet_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'leopet_contact7_form_content', 10, 2 );