...
function leopet_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'leopet' ),
        'next_string'     => esc_html__( '&raquo;', 'leopet' ),
        'before_output'   => '<div class="leopet-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'leopet_pagination_defaults', $defaults )
    );
